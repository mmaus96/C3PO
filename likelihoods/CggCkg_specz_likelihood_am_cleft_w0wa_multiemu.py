...
from likelihoods.background import classyBackground,classyBackground_taylor,classyBackground_w0wa
...
class CellLikelihood(Likelihood):
	...
	# Load the linear parameters of the theory model theta_a such that
        ...
        taylor_fid = [-1.0,0.0, 0.31519, np.log(1e10 * 2.0830e-9)] + fid_bias #w0,wa,Om,lnA
        ...
        self.cl_pred = limb(self.dndz, taylor_fid, None,None, None, classyBackground_w0wa, zmin=0.001, zmax=1.8, Nz=80,lmax=6000,taylor=False,taylor_args=None,zeffs=self.zfids)
...
class Taylor_Cells(Theory):
    ...
    def initialize(self):
        ...
        # pcleft_fns = [self.basedir + x for x in self.Pgm_filenames]
        # pmm_fn = self.basedir + self.Pmm_filename
        order = 4
        #Now load dndzs:
        dndzs     = [np.loadtxt(self.basedir + self.dndzfns[i]) for i in range(len(self.gal_sample_names))]
        self.dndz = pack_dndz(dndzs)
        self.Pgm_filenames = np.array(self.Pgm_filenames)
        self.cl_pred = {}
        for i,Pmm_fn in enumerate(self.Pmm_filenames):
            print(Pmm_fn,self.Pgm_filenames[:,i])
            taylor_args = [Pmm_fn,self.Pgm_filenames[:,0],order]
        
            # set up the theory prediction class.
            self.cl_pred[f'{i}'] = limb(self.dndz, taylor_fid, None,None, None, classyBackground_w0wa, zmin=0.001, zmax=1.8, Nz=80,lmax=6000,taylor=True,taylor_args=taylor_args,zeffs=self.zeffs)
    ...
    def calculate(self, state, want_derived=True, **params_values_dict):
        ...
        w0,wa,Om,logAs,Mnu = self.get_cosmo_parameters()
        ...
        for ii,gal_sample_name in enumerate(self.gal_sample_names):
            ...
            params = np.array([w0,wa,Om,logAs,b1,b2,bs])
            
            Cell_tables[gal_sample_name] = {}
            
            if self.do_auto:
                Cgg,Ckg = self.cl_pred[cl_ind].computeCggCkg(ii,params,smag)
                Cell_tables[gal_sample_name]['auto'] = Cgg
                Cell_tables[gal_sample_name]['cross'] = Ckg
            else:
                Ckg = self.cl_pred[cl_ind].computeCkg(ii,params,smag)
