...
from likelihoods.background import classyBackground,classyBackground_taylor
...
class CellLikelihood(Likelihood):
	...
	# Load the linear parameters of the theory model theta_a such that
        ...
        taylor_fid = [0.9649,0.02237, 0.1200,0.6736, np.log(1e10 * 2.0830e-9)] + fid_bias
        ...
        self.cl_pred = limb(self.dndz, taylor_fid, None,None, None, classyBackground_taylor, zmin=0.001, zmax=1.8, Nz=80,lmax=6000,taylor=False,taylor_args=None,zeffs=self.zfids)
...
class Taylor_Cells(Theory):
    ...
    def initialize(self):
        ...
        taylor_args = [self.Pmm_filename,self.Pgm_filenames,order]
        
        # set up the theory prediction class.
        self.cl_pred = limb(self.dndz, taylor_fid, None,None, None, classyBackground_taylor, zmin=0.001, zmax=1.8, Nz=80,lmax=6000,taylor=True,taylor_args=taylor_args,zeffs=self.zeffs)
    ...
    def calculate(self, state, want_derived=True, **params_values_dict):
        ...
        for ii,gal_sample_name in enumerate(self.gal_sample_names):
            ...
            params = np.array([ns,omb,omc,h,logAs,b1,b2,bs])
