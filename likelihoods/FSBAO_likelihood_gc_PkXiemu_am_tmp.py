...
#             self.sp_kmax[fs_sample_name] = self.kmax_spline[ll]
        ...
        # print(f'Current memory usage before loading Data: {process.memory_info().rss*1e-9} GB')
        self.loadData()
        print('Loaded data!')
        # process = psutil.Process()
        # print(f'Current memory usage after loading Data: {process.memory_info().rss*1e-9} GB')
    ...
    def logp(self,**params_values):
        ...
        # process = psutil.Process()
        # print(f'Current memory usage after FSBAO likelihood evaluation: {process.memory_info().vms*1e-9} GB')
...
class Taylor_pk_theory_zs(Theory):
    ...
    def initialize(self):
        ...
        # process = psutil.Process()
        # print(f'Current memory usage before loading emulators: {process.memory_info().rss*1e-9} GB')
        
        for ii, pk_filename in enumerate(self.pk_filenames):
            zfid = self.zfids[ii]
            zstr = "%.2f"%(zfid)
            taylors_pk = {}
            taylors_xi = {}
            ...
            xi_filename = self.xi_filenames[ii]
            ...
            x0s = emu['x0']
            rvec = emu['rvec']
            derivs_x0 = [np.array(ll) for ll in emu['derivs0'][:5]]
            derivs_x2 = [np.array(ll) for ll in emu['derivs2'][:5]]
            ...
            s8_filename = self.s8_filenames[ii]
            self.s8_emus[zstr] = Compute_zParams(self.basedir + s8_filename)
            
            del emu
        print('All emulators loaded!')
        # process = psutil.Process()
        # print(f'Current memory usage after loading emulators: {process.memory_info().rss*1e-9} GB')
    def get_requirements(self):
    ...
    def calculate(self, state, want_derived=True, **params_values_dict):
        ...
        cosmopars = [ns,omega_b, omega_cdm, hub, logA]
        ...
        for ii,zfid in enumerate(self.zfids):
            ...
            xi0table = taylor_approximate(cosmopars, x0s, derivs_xi0, order=4)
            xi2table = taylor_approximate(cosmopars, x0s, derivs_xi2, order=4)
        ...
        state['derived'] = {'sigma8': sig80,'omegam':OmM}
        state['zPars'] = zPars
        state['taylor_pk_ell_mod'] = ptables
        state['taylor_xi_ell_mod'] = xitables

        # process = psutil.Process()
        # print(f'Current memory usage after FSBAO Theory evaluation: {process.memory_info().rss*1e-9} GB')